...
def deconv3d(data,output_shape,scope,shape,stride=[1,1, 2, 2, 1],padding='SAME',wd=0.0,FLOAT16=False,reuse=None):
  with tf.variable_scope(scope, 'DeConv3d', [data], reuse=reuse):
    STDdev=1/tf.sqrt(shape[0]*shape[1]*shape[2]*shape[3]/2) #Xavier/2 initialization      
    kernel = _variable_with_weight_decay('weights',
                                         shape=shape,
                                         stddev=STDdev,
                                         wd=wd,FLOAT16=FLOAT16)
    deconv = tf.nn.conv3d_transpose(data, kernel,output_shape, stride, padding=padding)
    biases = _variable_on_cpu('biases', [shape[3]], tf.constant_initializer(0.00001))#positive biases
